...
export default function RSVPSection() {
  ...
  const handleSubmit = (e) => {
    ...
    // scroll (tanpa smooth biar ringan)
    setTimeout(() => {
  ...
  return (
    <section className="relative w-full min-h-fit bg-black text-white px-4 py-10 md:py-20">
      ...
      <div className="absolute top-0 left-0 w-96 h-96 bg-red-600/10 rounded-full blur-[128px]" />
      ...
      <div className="relative z-10 max-w-3xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          {/* Statistik Cards (tanpa whileHover) */}
          <div className="grid grid-cols-3 gap-3 mt-8">
            <div className="bg-red-950/30 backdrop-blur-sm border border-red-800/50 rounded-lg p-3">
            ...
            {isFormVisible && (
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {formStep === 1 && (
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 rounded-lg bg-black/50 border border-red-800/50 focus:border-red-500 focus:outline-none text-white"
                ...
                {formStep === 3 && (
                  <div>
                    <label className="block text-sm text-gray-400 mb-2">
                      Komentar & Doa (opsional)
                    </label>
                    <textarea
                      value={comment}
                      onChange={(e) => setComment(e.target.value)}
                      className="w-full px-4 py-3 rounded-lg bg-black/50 border border-red-800/50 focus:border-red-500 focus:outline-none resize-none"
                ...
                {submitted && (
                  <div className="text-center py-2 bg-green-600/20 border border-green-600 rounded-lg">
          ...
          {filteredGuests.length > 0 ? (
            <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
              {filteredGuests.map((guest, index) => {
                const isOpen = selectedGuest === (guest.id || index);
                return (
                  <div
                    key={guest.id || index}
                    onClick={() =>
                      setSelectedGuest(isOpen ? null : guest.id || index)
                    }
                    ...
                    } border border-red-800/50 cursor-pointer`}
                  >
                    ...
                    {/* komentar expand/collapse (tanpa AnimatePresence) */}
                    {isOpen && guest.comment && (
            ...
            <div className="text-center py-12 text-gray-500 border-2 border-dashed border-red-800/30 rounded-lg">
